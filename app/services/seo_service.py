...
def _readability_score(words, sentence_count):
    if not words:
        return 0.0
...
def _extract_keywords(title, content, tags):
    ...
    for word in _tokenize(tags.replace(",", " ")):
        normalized = _normalize_keyword(word)
...
def analyze_post_fields(title, content, meta_description="", tags="", category=""):
    ...
    readability_score = _readability_score(words, sentence_count)
    ...
    paragraph_count = len([paragraph for paragraph in content.split("\n\n") if paragraph.strip()])
    ...
    if primary_keyword and 0.5 <= primary_keyword_density <= 3.0:
        ...
        score += 8
        add_warning(
            f"Primary keyword density for `{primary_keyword['term']}` is {primary_keyword_density}%."
        )
        ...
        suggestions_json=json.dumps(
            {
                ...
                "primary_keyword_density": analysis["primary_keyword_density"],
            }
        ),
        keywords_json=json.dumps(analysis["keyword_candidates"]),
...
def get_latest_post_analysis(post):
    ...
    return serialize_report(latest_report)
